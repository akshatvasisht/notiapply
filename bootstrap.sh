...
if ! command -v tectonic &>/dev/null; then
  curl -fsSL https://github.com/tectonic-typesetting/tectonic/releases/latest/download/tectonic-aarch64-unknown-linux-musl.tar.gz \
    | tar xz -C /tmp
...
pip3 install --break-system-packages --quiet \
  jobspy requests playwright psycopg2-binary
...
if ! command -v dbmate &>/dev/null; then
  curl -fsSL https://github.com/amacneil/dbmate/releases/latest/download/dbmate-linux-arm64 \
...
if ! sudo -u postgres psql -tAc "SELECT 1 FROM pg_roles WHERE rolname='$DB_USER'" | grep -q 1; then
  ...
  echo "DB_PASS=$DB_PASS" > /opt/notiapply/.db_credentials
