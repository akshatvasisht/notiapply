-- Notiapply — Baseline Schema
-- Run once on initial setup. After this, use migrations/ for changes.

-- =============================================================================
-- user_config — single-row JSONB configuration store
-- =============================================================================
...
-- =============================================================================
-- pipeline_modules — module registry for the n8n orchestrator
-- =============================================================================
...
CREATE TABLE pipeline_modules (
  ...
  phase            TEXT NOT NULL
    CHECK (phase IN ('scraping', 'processing', 'output')),
...
CREATE INDEX idx_pipeline_modules_phase_order
  ON pipeline_modules(phase, execution_order);

-- Built-in module seeds (permanent — can be toggled, not deleted)
...
('scrape-jobspy',
 ...
 '{
    ...
    "properties": {
      "sources": {
        "type": "array", "title": "Active sources",
        "items": {"type": "string",
          "enum": ["linkedin","indeed","glassdoor","zip_recruiter"]}
      },
      "results_per_source": {"type": "number", "title": "Results per source", "default": 50}
    }
  }',
 '{}'),
...
-- =============================================================================
-- master_resume — insert-only versioning
-- =============================================================================
...
-- =============================================================================
-- cover_letter_templates — insert-only versioning
-- =============================================================================
...
-- =============================================================================
-- applications — one per job, tracks fill state and PDF bytes
-- =============================================================================
...
-- =============================================================================
-- resume_diffs — LLM response audit trail
-- =============================================================================
...
-- =============================================================================
-- fill_sessions — sidecar session tracking
-- =============================================================================
...
-- =============================================================================
-- scraped_companies — ATS direct watchlist
-- =============================================================================
...
-- =============================================================================
-- github_poll_state — commit tracking per repo
-- =============================================================================
...
-- =============================================================================
-- ats_failure_counts — derived view, not a table
-- =============================================================================
